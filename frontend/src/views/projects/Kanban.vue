        ...
        >
          ...
          >
            ...
            <draggable
              ...
              ghost-class="ghost"
              chosen-class="chosen"
              @start="onDragStart($event, column.status)"
              @end="onDragEnd"
            >
...
const onDragStart = (evt: any, fromStatus: TaskStatus) => {
  pendingDrag.value = {
    taskId: evt.item?.__draggable_context?.element?.id,
    fromStatus,
    toStatus: fromStatus,
  };
};
...
const onDragEnd = async (evt: any) => {
  if (!pendingDrag.value || !kanbanData.value) return;
  const { taskId, fromStatus } = pendingDrag.value;

  const toColumn = kanbanData.value.columns.find((c) =>
    c.tasks.some((t: any) => t.id === taskId),
  );
  const toStatus = toColumn?.status ?? fromStatus;
  pendingDrag.value = null;

  if (fromStatus === toStatus) return;

  try {
    await taskApi.updateStatus(taskId, toStatus);
