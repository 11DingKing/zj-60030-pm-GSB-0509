...
export class StatisticsService {
  ...
  async getBurnDownChart(sprintId: string, userId: string) {
    ...
    const totalStoryPoints = sprint.tasks.reduce((sum, t) => sum + t.storyPoints, 0);
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      const idealRemaining = totalStoryPoints - (totalStoryPoints / (totalDays - 1)) * i;
      idealPoints.push({ date: dateStr, remaining: Math.max(0, idealRemaining) });

      let actualRemaining = totalStoryPoints;
      sprint.tasks.forEach(task => {
        const doneLog = task.statusLogs.find(
          log => log.newStatus === TaskStatus.DONE && 
  ...
  async getVelocityTrend(projectId: string, userId: string) {
    ...
    const velocityData = project.sprints.map(sprint => {
      ...
      const plannedPoints = sprint.tasks.reduce((sum, t) => sum + t.storyPoints, 0);
    ...
    const result = {
      ...
      averageVelocity: velocityData.length > 0
        ? velocityData.reduce((sum, d) => sum + d.completedPoints, 0) / velocityData.length
        : 0,
    };
  ...
  async getBugTrend(projectId: string, userId: string) {
    ...
    const bugData = project.sprints
      ...
      .map(sprint => {
        ...
        const resolvedBugs = sprint.tasks.filter(t => t.status === TaskStatus.DONE).length;
